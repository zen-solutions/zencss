...
document.querySelector('.next').addEventListener('click', () => {
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      ...
      slidesContainer.style.transform = `translateX(-${index1 * slideWidth}px)`;
    });
  });
});
...
document.querySelector('.prev').addEventListener('click', () => {
  if (index1 > 0) {
    ...
    index1 = totalSlides - 2; // Move to the last original slide
    slidesContainer.style.transition = 'none';
...
//  // Make sure your HTML has a button with id="btn-modal" if you want to use a button to trigger the modal
...
//  // Uncomment the next lines if you have a button for opening the modal
...
//  // Timer to automatically show the modal after 7 seconds
// //  setTimeout(function() {
// //      modal.style.display = "block";
// //  }, 7000);
 
//  // Trigger the modal when the user moves the mouse to the top of the page, as if to leave
//  document.addEventListener('mousemove', function(e) {
//      if(e.clientY <= 5) {
//          // Avoid opening the modal if it's already displayed
